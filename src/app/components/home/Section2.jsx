...
const LazyBackground = ({ children, image, placeholder }) => {
  ...
  return (
    <div ref={ref} className="card lg:w-1/2 rounded-lg 2xl:rounded-2xl overflow-hidden bg-cover bg-bottom relative" style={{ backgroundImage: `url(${backgroundImage})` }}>
      {children}
    </div>
  );
};
...
const Card = ({ title, backgroundImage, titleColor }) => {
  ...
  return (
    <LazyBackground image={backgroundImage} placeholder="url('/placeholder-image.jpg')">
