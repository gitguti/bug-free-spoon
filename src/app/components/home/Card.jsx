...
const Card = ({ title, backgroundImage, titleColor, delay, isVisible, content, button }) => {
  ...
  const mobileAnimationClass = isVisible
    ...
    const contentClasses = `absolute font-regular top-20 lg:top-24 xl:top-28 3xl:top-36 4xl:top-40 left-4 md:left-6 xl:left-[2.2rem] text-xs md:text-sm md:leading-[22px] xl:text-base 3xl:text-xl  4xl:text-[1.75rem] 4xl:leading-[2.1rem] 2xl:p-8 w-3/4 lg:w-1/2 lg:w-2/3 xl:w-3/4 3xl:w-4/5 ${titleColor}`;
    ...
    return (
      <div className={`card lg:w-1/2 rounded-lg 2xl:rounded-2xl overflow-hidden bg-cover bg-bottom relative opacity-0 ${animationClass} ${mobileAnimationClass}`} style={{ backgroundImage: `url(${backgroundImage})` }}>
        ...
        <div className="absolute left-4 md:left-6 xl:left-8 3xl:left-16 top-48 sm:top-44 md:top-56 3xl:top-[19rem] 4xl:top-96">
